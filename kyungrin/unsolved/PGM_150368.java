...
public class PGM_150368 {
  ...
  static ArrayList[] usersBought; // temp, idx: 유저 번호, value: 유저가 구매한 이모티콘 번호
  //
  static int maxCnt;
  static int maxSale;
  ...
  private static void dc(int idx, int start) {
    if(idx == emtCnt){
    ...
    for(int i = start; i < 40; i += 10){
      ...
      for(int j = 0; j < userCnt; j++) {
        ...
        int ratio = info[0];
        if (i >= ratio) {
          // 2. 이상이라면 usersBought에 이모티콘 번호(idx) 추가
          usersBought[j].add(idx); // 어 이러면 데이터 더러워질 것 같은데
        }
      }

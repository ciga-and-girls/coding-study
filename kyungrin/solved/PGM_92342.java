...
public class PGM_92342 {
  class Solution {
    ...
    public int[] solution(int n, int[] info) {
      ...
      if(diff == Integer.MIN_VALUE) lionInfo = new int[]{-1};
    ...
    private static void subset(int score, int remainArrowCnt){
      if(score < 0) {
        // AI 사용 : 점수 차이가 같을 경우, 0점에 배치된 화살의 개수가 영향을 미친다.
        // 남은 화살이 있다면 0번 점수판에 넣는다.
        if(remainArrowCnt != 0) {
          tempLionInfo[10] += remainArrowCnt;
        }
          ...
          // 3. 같다면 lionInfo가 tempLionInfo 점수 idx를 뒤부터 비교하면서 값이 더 큰 쪽으로 변경
            ...
            for(int i = 10; i >= 0; i--){
              if(tempLionInfo[i] > lionInfo[i]){
                // AI 사용 : 얕은 복사 -> 깊은 복사
                for(int j = 0; j < 11; j++){
                  lionInfo[j] = tempLionInfo[j];
                }
                break;
              }
              else if(tempLionInfo[i] < lionInfo[i]) break;
            }
          }
        }
